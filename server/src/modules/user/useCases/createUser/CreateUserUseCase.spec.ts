...
describe("CreateUserUseCase", () => {
  ...
  it("should not create an user if email is already in use", async () => {
    ...
    const userWithDuplicatedEmail = () =>
      createUserUseCase.execute(newUserInfo);

    // Assert
    await expect(userWithDuplicatedEmail).rejects.toThrow(
  ...
  it.each(["123", "1234567", "abcdefg", "", ".", " "])(
    "should not create an user if password is less than 8 characters: $0",
    async (passwordValue) => {
      // Arrange
      const newUserInfo = {
        name: "John",
      ...
      // Act
      const createUserWithInvalidPassword = () =>
        createUserUseCase.execute(newUserInfo);

      // Assert
      await expect(createUserWithInvalidPassword).rejects.toThrow(
        "Senha invalida. Minimo 8 caracteres e maximo 16",
      );
  ...
  it.each(["123123123123123123", "abcdefghijklmnopqr"])(
    "should not create an user if password is more than 16 characters: $0",
    async (passwordValue) => {
      // Arrange
      const newUserInfo = {
        name: "John",
  ...
  it("should not create an user if password is missing", async () => {
    ...
    // Assert
    await expect(userWithoutPassword).rejects.toThrow(
      "Senha invalida. Minimo 8 caracteres e maximo 16",
    );
    expect(createUserRepositorySpy).not.toHaveBeenCalled();
    expect(inMemoryUserReposiory.users.length).toBe(0);
  ...
  it.each(["123", "john123", "j0hn", "123john"])(
    "should not create an user if name is invalid: $0",
    async (values) => {
      // Arrange
      const newUserInfo = {
        name: values,
